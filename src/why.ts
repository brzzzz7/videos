/**
 * The visagisme-origin reel (composition "Why").
 *
...
import spansJson from "./data/why-spans.json";
...
import { cues, emphasis, phrases, type Cue } from "./data/why";
...
frameCues.forEach((cue, i) => {
  ...
  if (previous && previous.sfx === cue.sfx) {
    throw new Error(
      `why: cue ${i} ("${cue.scene.kind}") repeats the sound "${cue.sfx}" ` +
        `right after cue ${i - 1} ("${previous.scene.kind}")`,
    );
  }
  if (previous && cue.from < previous.to) {
    throw new Error(
      `why: cue ${i} ("${cue.scene.kind}") starts before cue ${i - 1} ends`,
    );
  }
});
...
frameCues.forEach((cue, i) => {
  ...
  if (previous && cue.from - previous.to < MIN_FACECAM) {
    throw new Error(
      `why: only ${cue.from - previous.to} frames of facecam between cue ` +
