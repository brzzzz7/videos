...
import {
  ...
  Salon,
  Yes,
} from "./components/illos/secrets";
import { ramp } from "./lib/audio";
import type { Scene } from "./data/secrets";
import { hook } from "./data/secrets";
import {
  clips,
  ctaFrame,
  cueMark,
...
const Illustration: React.FC<{ scene: Scene; at: (s: number) => number }> = ({
  ...
  switch (scene.kind) {
    ...
    case "salon":
      return <Salon at={at} />;
    case "morning":
      return <Morning at={at} />;
    case "everyone":
      return <Everyone at={at} />;
    case "cta":
      return <CtaSecrets at={at} />;
  }
};
...
export const HOOK_FRAMES = 112;   // 3.7 s — it clears before the first scene
 ...
 * to frame 67; the sentence that follows it, "je vais t'en casser 3",
 * gets its own caption underneath while the hook is still up.
 *
...
const HookCard: React.FC = () => {
  ...
  // "il y a des trucs que les barbiers" 0-32, "ne te disent jamais" 32-52,
  // "au rendez-vous" 52-67, then "je vais t'en casser 3" to 90.
  const first = pop(1);
  const punch = pop(33);
  const rest = pop(53);
  // the marker sweeps across while he says "ne te disent jamais"
  const swipe = interpolate(frame, [35, 52], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
...
export const SecretsReel: React.FC = () => {
  return (
