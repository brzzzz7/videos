/**
 * The "what they never tell you" reel (composition "Secrets").
 *
...
import spansJson from "./data/secrets-spans.json";
...
import { cues, emphasis, phrases, type Cue } from "./data/secrets";

export const FPS = 30;
export const SOURCE_DURATION = 33.18;

/**
 * No speed-up: 33.2 s comes in at 30.0 s on pause-cutting alone, and no cap
 * was asked for.
 */
export const RATE = 1;
...
frameCues.forEach((cue, i) => {
  ...
  if (previous && previous.sfx === cue.sfx) {
    throw new Error(
      `secrets: cue ${i} ("${cue.scene.kind}") repeats the sound "${cue.sfx}" ` +
        `right after cue ${i - 1} ("${previous.scene.kind}")`,
    );
  }
  if (previous && cue.from < previous.to) {
    throw new Error(
      `secrets: cue ${i} ("${cue.scene.kind}") starts before cue ${i - 1} ends`,
    );
  }
});
...
frameCues.forEach((cue, i) => {
  ...
  if (previous && cue.from - previous.to < MIN_FACECAM) {
    throw new Error(
      `secrets: only ${cue.from - previous.to} frames of facecam between cue ` +
