/**
 * The three-mistakes reel (composition "Errors").
 *
...
import spansJson from "./data/errors-spans.json";
import {
  buildClips,
  groupLines,
  holdLines,
...
import { cues, emphasis, phrases, type Cue, type Sfx } from "./data/errors";
...
frameCues.forEach((cue, i) => {
  ...
  if (previous && previous.sfx === cue.sfx) {
    throw new Error(
      `errors: cue ${i} ("${cue.scene.kind}") repeats the sound "${cue.sfx}" ` +
        `right after cue ${i - 1} ("${previous.scene.kind}")`,
    );
  }
  if (previous && cue.from < previous.to) {
    throw new Error(
      `errors: cue ${i} ("${cue.scene.kind}") starts before cue ${i - 1} ends`,
    );
  }
});
...
frameCues.forEach((cue, i) => {
  ...
  if (previous && cue.from - previous.to < MIN_FACECAM) {
    throw new Error(
      `errors: only ${cue.from - previous.to} frames of facecam between cue ` +
