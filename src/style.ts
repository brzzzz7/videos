/**
 * The trending-cut reel (composition "Style").
 *
...
/**
 * Global speed-up. Cutting the pauses as hard as they take gets this take from
 * 26 s to 19.4 s across 14 clips, which is most of the pace; the last second
 * and a half to reach the 18 s cap comes from tempo. The voice is retimed in
 * its own file (`VOICE_TEMPO=1.10` on build-voice.py, i.e. `atempo`, so the
...
frameCues.forEach((cue, i) => {
  ...
  if (previous && previous.sfx === cue.sfx) {
    throw new Error(
      `style: cue ${i} ("${cue.scene.kind}") repeats the sound "${cue.sfx}" ` +
        `right after cue ${i - 1} ("${previous.scene.kind}")`,
    );
  }
  if (previous && cue.from < previous.to) {
    throw new Error(
      `style: cue ${i} ("${cue.scene.kind}") starts before cue ${i - 1} ends`,
    );
  }
});
...
frameCues.forEach((cue, i) => {
  ...
  if (previous && cue.from - previous.to < MIN_FACECAM) {
    throw new Error(
      `style: only ${cue.from - previous.to} frames of facecam between cue ` +
