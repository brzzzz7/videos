/**
 * The client-stories reel (composition "Stories").
 *
...
import spansJson from "./data/stories-spans.json";
import {
  buildClips,
  groupLines,
  holdLines,
...
import { cues, emphasis, phrases, type Cue, type Sfx } from "./data/stories";
...
frameCues.forEach((cue, i) => {
  ...
  if (previous && previous.sfx === cue.sfx) {
    throw new Error(
      `stories: cue ${i} ("${cue.scene.kind}") repeats the sound "${cue.sfx}" ` +
