...
import {
  ...
  Refus,
  TwoFaces,
} from "./components/illos/copy";
import { ramp } from "./lib/audio";
import type { Scene } from "./data/copy";
 ...
 * to frame 71, which is where the captions take over.
 *
...
const HookCard: React.FC = () => {
  ...
  // "on me demande souvent" 0-23, "la même coupe" 23-40, the rest to 71.
  const first = pop(1);
  const punch = pop(24);
  const rest = pop(41);
  // the marker sweeps across while he says "la même coupe"
  const swipe = interpolate(frame, [27, 40], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
...
export const CopyReel: React.FC = () => {
  return (
